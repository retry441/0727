...
int open_database(char *file_name)
{
	extern sqlite3 *db;
...
int search_data(sqlite3 *db,char *table_name,char *search_column_name,char *search_content,int cmd)
{
    ...
    {	
    	if(strcmp(search_column_name,"id1") == 0)
    	{
    		temp_id = atoi(search_content);
    		sprintf(sql,"select * from %s where id1=%d;",table_name,temp_id);
    	}
...
int sqlite3_init()
{
	...
	create_table(db,"filelist",2);//文件名 发送者id
